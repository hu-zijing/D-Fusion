...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        self.has_attn_probs = False
        self.attn_dim = None
        self.device = None
        self.dtype = None
        self.is_set_temp_attn = False
        self.temp_attn = None
    ...
    def forward(
        ...
        **cross_attention_kwargs,
        ...
        hidden_states, attention_probs = self.processor(
            self,
            hidden_states,
            encoder_hidden_states=encoder_hidden_states,
            attention_mask=attention_mask,
            given_attn_map = given_attn_map, 
    ...
    def get_attention_scores(
        ...
        # print(baddbmm_input.shape, query.shape, key.shape)
        # print(baddbmm_input.dtype, query.dtype, key.dtype)
...
class AttnProcessor:
    ...
    def __call__(
        ...
        if is_cross or not (given_attn_map is not None and inject_component[0]): 
            attention_probs = attn.get_attention_scores(query, key, attention_mask)
            hidden_states = torch.bmm(attention_probs, value)
        else : 
            query2 = given_attn_map[0]
            key2 = given_attn_map[1]
            value2 = given_attn_map[2]

            new_mask = None
            new_mask2 = None
            if given_cross_mask is not None:
                # src mask
                new_mask = (given_cross_mask[0] > 0.5).any(dim=2).half()
                height0_width0 = query.shape[1]
                height0 = width0 = int(height0_width0**0.5)
                # print(new_mask.shape, height0_width0)
                bsize, height_width = new_mask.shape
                height = width = int(height_width**0.5)
                new_mask = new_mask.reshape(bsize, height, width)
                new_mask = torch.cat([new_mask]*2)
                new_mask = F.interpolate(new_mask.unsqueeze(1), (height0, width0)).reshape(2*bsize, height0_width0)
                new_mask = new_mask.unsqueeze(1).repeat(1, attn.heads, 1).reshape(2*bsize*attn.heads, height0_width0)
                
            if new_mask is None: 
                attention_probs = attn.get_attention_scores(query, key2, attention_mask)
                hidden_states = torch.bmm(attention_probs, value2)
            else: 
                # fg bg
                mask1 = new_mask.unsqueeze(2)
                mask2 = new_mask.unsqueeze(2)
                attention_probs = attn.get_attention_scores(query, key2*mask1+key*(1-mask2))
                hidden_states = torch.bmm(attention_probs, value2*mask1+value*(1-mask2))
        ...
        if is_cross: 
            if attention_probs.shape[1] == used_layer_size*used_layer_size: 
                return hidden_states, attention_probs
            else: 
                return hidden_states, None # attention_probs
        else: 
            return hidden_states, (query if inject_component[0] else None, 
                key if inject_component[1] else None, 
                value if inject_component[2] else None)
