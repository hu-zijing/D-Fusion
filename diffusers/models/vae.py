    ...
    def forward(self, sample: torch.FloatTensor) -> torch.FloatTensor:
        ...
        if self.training and self.gradient_checkpointing:
            ...
            if is_torch_version(">=", "1.11.0"):
                ...
                for down_block in self.down_blocks:
                    sample, _ = torch.utils.checkpoint.checkpoint(create_custom_forward(down_block), sample)
                # middle
                sample, _ = torch.utils.checkpoint.checkpoint(create_custom_forward(self.mid_block), sample)
            ...
            # middle
            sample, _ = self.mid_block(sample)
    ...
    def forward(
        ...
        if self.training and self.gradient_checkpointing:
            ...
            if is_torch_version(">=", "1.11.0"):
                ...
                # up
                for up_block in self.up_blocks:
                    sample, _ = torch.utils.checkpoint.checkpoint(create_custom_forward(up_block), sample, latent_embeds)
        else:
            # middle
            sample, _ = self.mid_block(sample, latent_embeds)
            sample = sample.to(upscale_dtype)

            # up
            for up_block in self.up_blocks:
                sample, _ = up_block(sample, latent_embeds)
